...
- G5 build (opus ×1, sequential): P1–P6 at `4bcf153`…`9a38072`, pushed; suite 229/2 (baseline 173/2). `tools/additivity_run.py` reanalysis 140.5 min → `reports/additivity_real.json` (18 candidates). P7 docs left uncommitted by the first builder (parked on 10-min waiters, ~8 wakes at 375k tokens — lesson: one Monitor in the orchestrator, builders never wait).
- G6 round 1 on `main..9a38072` (opus ×2 fresh): conformance — every P1–P6 item, AM1–AM12, D1–D7 present; adversarial — no numerical defects (ceiling = scipy to 1e-16 on 9 count vectors; fixtures go red under mutation; level projection orthonormal, complement leakage 2.5e-8). 9 structural findings → 8.1 round 1.
- G7 partial (orchestrator): ceiling report recomputed independently — 7 families + 17 candidates match, oracle 3/7; suite 229/2 run by me; level-split table read; additivity guard's "6 differing" = the 6 NaN `hiss+bandlimit@2` cells at 16 kHz, all 162 cells otherwise bit-identical to `combos_real.json`.
- G8.1 round 1 (fresh opus): P7 `f2fd41d` + R1–R8 `3a9d6cf`…`29b1e12`, pushed; suite 269/2; `--scan-duplicates` 0 of 400,350 keys; verify_guard identical on 2 candidates. Two unrequested judgments reported (`reason="no_common_source"`, `candidate_block()` extraction).
- G6 round 2 dispatched on `9a38072..29b1e12` (opus ×2 fresh) with the external auditor's uncommitted D01–D13 list as a coverage target. G7 finding for round 2: S6 text says "seven candidates" (18), β range 0.04–0.40, hum+mp3 cos_to_b to 1.00, norm_ratio from 0.07; no "what stands" line for hiss+bandlimit.
